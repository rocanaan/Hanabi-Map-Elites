...
public class TestRuleAgent implements Agent {
    ...
    private Rule testRule = new TellUnambiguous();
    ...
    public Action doMove(int playerID, GameState gameState) {
        ...
        System.out.println("Can test rule fire? " + testRule.canFire(playerID, gameState));
        System.out.println(possibleMoves.size() + " test Rule");
...
//        System.out.println(r.couldFire(1, gameState));
        ...
        if (moveToMake == possibleMoves.size()) {
        		return testRule.execute(playerID, gameState); 
        }
