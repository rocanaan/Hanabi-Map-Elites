...
public class StandardDeviationEstimation 
{
    public static void main( String[] args )
    {
        ...
        // Best agent on the GA
 		...
 		HistogramAgent HailMaryHAgent = RulebaseExtended.makeAgent(rules3);
 		...
 		for (int i = 0; i < chromossome4.length; i++) {
 			rules4[i] = RulebaseExtended.ruleMapping(chromossome4[i]);
 		}
 		HistogramAgent evolvedMixed1 = RulebaseExtended.makeAgent(rules4);
 		...
 		for (int i = 0; i < chromossome5.length; i++) {
 			rules5[i] = RulebaseExtended.ruleMapping(chromossome5[i]);
 		}
 		HistogramAgent evolvedMixed2 = RulebaseExtended.makeAgent(rules5);
 		...
 		for (int i = 0; i < chromossome6.length; i++) {
 			rules6[i] = RulebaseExtended.ruleMapping(chromossome6[i]);
 		}
 		HistogramAgent evolvedMixed5 = RulebaseExtended.makeAgent(rules6);
 		...
 		for (int i = 0; i < chromossome7.length; i++) {
 			rules7[i] = RulebaseExtended.ruleMapping(chromossome7[i]);
 		}
 		HistogramAgent evolvedMixed4 = RulebaseExtended.makeAgent(rules7);
 		...
 		for (int i = 0; i < chromossome8.length; i++) {
 			rules8[i] = RulebaseExtended.ruleMapping(chromossome8[i]);
 		}
 		HistogramAgent evolvedMirror2= RulebaseExtended.makeAgent(rules8);
 		...
 		for (int i = 0; i < chromossome9.length; i++) {
 			rules9[i] = RulebaseExtended.ruleMapping(chromossome9[i]);
 		}
 		HistogramAgent evolvedMirror3= RulebaseExtended.makeAgent(rules9);
...
// 		population.add(new AgentPlayer("Partial Evolved Mixed 4", PartialEvolvedMixed4));
 		...
 		Vector<AgentPlayer> baseline = RulebaseExtended.GetBaselineAgentPlayers();
