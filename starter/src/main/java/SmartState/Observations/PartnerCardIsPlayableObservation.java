...
public class PartnerCardIsPlayableObservation extends SmartStateObservation{
	
	
	public PartnerCardIsPlayableObservation() {
	...
	public List<Double> getObservation(GameState state, int playerID) {
		List<Double> partnerCardIsPlayable = new ArrayList<Double>();
		...
		for (int i = 1; i<state.getPlayerCount(); i++){
			...
			for (int c = 0; c<state.getHandSize(); c++) {
				...
				if (gse.isPlayable(card, state)){
					partnerCardIsPlayable.add(1.0);
				}
				else {
					partnerCardIsPlayable.add(0.0);
				}
			}
		}
		
		return partnerCardIsPlayable;
	}
