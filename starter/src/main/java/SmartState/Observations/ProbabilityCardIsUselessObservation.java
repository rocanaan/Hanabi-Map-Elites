...
public class ProbabilityCardIsUselessObservation extends SmartStateObservation{
	
	
	public ProbabilityCardIsUselessObservation() {
	...
	public List<Double> getObservation(GameState state, int playerID) {
		List<Double> uselessProbabilities = new ArrayList<Double>();
		
		for (int i = 0; i<state.getPlayerCount(); i++){
			int targetID = (playerID+i)%state.getPlayerCount();
			GamestateEvaluator gse = new GamestateEvaluator(playerID, state);
			uselessProbabilities.addAll(gse.getProbabiltyUseless(targetID));
		}
		
		return uselessProbabilities;
	}
