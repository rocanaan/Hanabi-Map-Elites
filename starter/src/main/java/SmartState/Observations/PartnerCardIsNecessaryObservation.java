...
public class PartnerCardIsNecessaryObservation extends SmartStateObservation{
	
	
	public PartnerCardIsNecessaryObservation() {
	...
	public List<Double> getObservation(GameState state, int playerID) {
		List<Double> partnerCardIsUseless = new ArrayList<Double>();
		...
		for (int i = 1; i<state.getPlayerCount(); i++){
			...
			for (int c = 0; c<state.getHandSize(); c++) {
				...
				if (gse.isUseless(card, state)){
					partnerCardIsUseless.add(1.0);
				}
				else {
					partnerCardIsUseless.add(0.0);
				}

			}
		}
		
		return partnerCardIsUseless;
	}
