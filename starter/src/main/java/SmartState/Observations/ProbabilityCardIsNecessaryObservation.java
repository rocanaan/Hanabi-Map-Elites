...
public class ProbabilityCardIsNecessaryObservation extends SmartStateObservation{
	
	
	public ProbabilityCardIsNecessaryObservation() {
	...
	public List<Double> getObservation(GameState state, int playerID) {
		List<Double> necessaryProbabilities = new ArrayList<Double>();
		
		for (int i = 0; i<state.getPlayerCount(); i++){
			int targetID = (playerID+i)%state.getPlayerCount();
			GamestateEvaluator gse = new GamestateEvaluator(playerID, state);
			necessaryProbabilities.addAll(gse.getProbabiltyNecessary(targetID));
		}
		
		return necessaryProbabilities;
	}
